r"""Slice 12 stage A: screen the three experimental Rust pair emitters.
...
what the table reports.
    ...
    pixi run python benchmarks/bench_pair_emitters.py --repeat 3 \\
        --out benchmarks/reports/pair_emitters_screening.json

Before timing anything the driver proves the emitters agree with the Python
matrix oracle: for the ``--parity`` fixtures it computes
``relationship_pairs(max_degree=5)`` on the graph and on a deterministic
half view and compares per-block counts and digests with every emitter's
...
from _pair_fixtures import BINARY, EMITTERS, Inputs

from pedigree_graph import RELATIONSHIPS


def run_binary(tsv: Path, emitter: str, threads: int, view: Path | None, degree: int) -> dict:
    """One fresh-process run of the benchmark binary, as its JSON record."""
    cmd = [str(BINARY), str(tsv), "--emitter", emitter, "--threads", str(threads), "--max-degree", str(degree)]
    if view is not None:
        cmd += ["--view", str(view)]
...
def check_parity(name: str, inputs: Inputs) -> dict[str, dict[str, list[int]]]:
    """Every emitter's blocks must equal the in-tree oracle's on the graph and on the view."""
    graph = inputs.graph
    expected = {
    ...
    for receiver, want in expected.items():
        for emitter in EMITTERS:
            got = run_binary(inputs.tsv, emitter, 2, inputs.view if receiver == "view" else None, 5)["blocks"]
            bad = [code for code in RELATIONSHIPS if got[code] != want[code]]
            if bad:
                detail = ", ".join(f"{c}: rust {got[c]} oracle {want[c]}" for c in bad)
                raise SystemExit(f"{name}/{receiver}/{emitter} disagrees with the oracle: {detail}")
        print(f"parity ok: {name}/{receiver}, {sum(v[0] for v in want.values())} pairs", flush=True)
    return expected


def summarise(runs: list[dict]) -> list[dict]:
    """Medians and ranges per (fixture, receiver, threads, emitter) cell."""
    cells: dict[tuple, list[dict]] = {}
    for run in runs:
        cells.setdefault((run["fixture"], run["receiver"], run["threads"], run["emitter"]), []).append(run)
    rows = []
    for (fixture, receiver, threads, emitter), group in sorted(cells.items()):
...
def render(rows: list[dict]) -> str:
    ...
    lines.extend(
        f"| {r['fixture']} | {r['receiver']} | {r['threads']} | {r['emitter']} | {r['pairs']:,} | "
...
def main() -> None:
    ...
    ap.add_argument("--emitters", nargs="+", default=list(EMITTERS))
    ...
    work = args.work_dir or Path(tempfile.mkdtemp(prefix="pair-emitters-"))
            ...
            for receiver in args.receivers:
                ...
                for threads in args.threads:
                    for emitter in args.emitters:
                        record = run_binary(inputs[name].tsv, emitter, threads, view, args.degree)
                        record.update({"fixture": name, "receiver": receiver, "repeat": rep})
                        runs.append(record)
                        print(
                            f"[{rep}] {name}/{receiver}/t{threads}/{emitter}: {record['seconds']:.3f}s, "
    ...
    rows = summarise(runs)
    report = {
        "benchmark": "pair_emitters_screening",
