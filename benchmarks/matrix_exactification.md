...
pixi run python benchmarks/matrix_exactification.py --repeat 5 --out benchmarks/reports/sweep.json
pixi run python benchmarks/matrix_exactification.py --render benchmarks/reports/sweep.json
```

This note is a frozen slice 5b record. Since slice 14 the numba DP the script
drives lives in the Rust core, so these commands run only at a pre-slice-14
commit such as `81e75d5826`, the parent of the slice 14 move `2d8b958`. The
live benchmark of the matrix products is `benchmarks/bench_kinship_matrix.py`.
...
the script's `--render` mode, not typed.
...
- the four relationship rows were added later and ran at commit `2f282adbbd`,
  harness `b7bf3533c18159ba`; that revision only adds those configs and leaves the
...
| fitACE `dev_cont_n10k/rep1` | relationship support, max_degree=3 (public path) | 3 | 3.23 s | 2.5% | 437 MiB | `1060716544` |
| fitACE `dev_cont_n10k/rep1` | relationship support, max_degree=5 (public path) | 3 | 26.86 s | 2.7% | 692 MiB | `1011761152` |
| generated `random_30k` | relationship support, max_degree=3 (public path) | 3 | 84.60 s | 0.1% | 808 MiB | `12021248` |
| generated `random_30k` | relationship support, max_degree=5 (public path) | 3 | 264.13 s | 0.0% | 1,550 MiB | `56619520` |
...
chunks, and that choice is now measured rather than assumed. The largest
realistic selector, `max_degree=5` on `random_30k`, takes 264.13 s at 1,550
MiB through the public path.

An earlier draft of this note claimed that support "approaches the size that
produced the 6,745 s result". That was wrong. `random_30k` has 3,395,984
degree-5 pairs against 26,924,109 candidates at `0.001`, so the relationship
support is 7.9x smaller and fits roughly four 1,048,576-pair chunks instead of
26. Fewer chunks means less dependency rediscovery, which is the cost that
makes the dense case quadratic in practice.

Pair chunks are therefore the right strategy for relationship support and there
is no cliff to remove. The boundary worth recording is that the strategy is
appropriate because the support is sparse, not because chunking is cheap: a
selector that ever approached the density of the `0.001` candidate set would
belong on the fused path instead.
