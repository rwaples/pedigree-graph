...
| `_relationship_counts.py` | `relationship_counts` (graph) and `view_relationship_counts` (view, as a boolean row mask): selector validation shared with the pair path, the thread budget, one call into `_native.relationship_counts` over the graph's own five columns at the degree of the highest requested code, and the typed exact `RelationshipCountResult`. No pair list, no native state retained (ADR [0010](adr/0010-row-streaming-relationship-engine.md) as amended). |
