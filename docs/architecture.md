...
| `_kinship_depth.py`, `_inbreeding_kernel.py` | The two numba kernels left on the kinship side: Maignel's equivalent complete generations, and the genome-node Meuwissen-Luo inbreeding walk (ADR 0008). The kinship DP itself runs in the core; its 0.9.1 Python form is the differential oracle `tests/oracle/kinship_dp/`. |
