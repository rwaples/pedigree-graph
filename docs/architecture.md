...
|---|---|
| `_core.py` | The `PedigreeGraph` class: graph data, parent CSR, kinship/inbreeding, the canonical `from_frame` / `from_arrays` constructors, and thin receiver methods for views, relationship operations, and pair kinship. Matrix receiver methods live with their implementation in `_kinship_matrix.py`; input validation lives in `_input.py`. `_from_input` is the one builder every entry point funnels through. |
...
| `crates/core/src/` | Rust `pedigree-graph-core`: `topology` (depth, depth-major order, cycle witness), `error` (structured codes), and `relationships`, the row-streaming exact relationship engine with the `pgr-count` CLI (ADR [0010](adr/0010-row-streaming-relationship-engine.md)). Parity fixtures under `crates/core/tests/fixtures/` come from `tests/parity/dump_relationship_inputs.py`. |
