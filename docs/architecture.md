...
| `_view.py` | `PedigreeView` and `CoordinateToken`: the `PedigreeGraph.view(ids=...)` / `view(rows=...)` selection boundary (reusing the `_input.py` coercion helpers), the owned read-only `ids` / `graph_rows` arrays, and the opaque per-receiver coordinate token (ADR [0006](adr/0006-public-api-and-coordinate-semantics.md)). |
| `_compat.py` | The bodies of the 0.7.1 `from_subsample` and `extract_pairs` (`legacy_extract_pairs`: 0.7.1 code selection, `(min, max)` collateral orientation, sample mask, caller remap, count cache), kept apart so 0.8.0 removes them by deleting the module. |
...
| `relationships.py` | The public relationship vocabulary and result types: re-exports `RELATIONSHIPS` / `RelationshipCategory` from `_registry.py` and defines `RelationshipPairBlock` (owned read-only int32 `first_rows` / `second_rows`, roles, `requested`, private receiver token) and the immutable 23-key `RelationshipPairs` mapping (ADR [0006](adr/0006-public-api-and-coordinate-semantics.md)). |
| `_pair_utils.py` | Free functions shared by the pair engines: `canonical_keys` / `sort_by_canonical_key` (the unordered `min * n + max` key), `subtract_pairs` (the one canonical-key membership subtraction), `oriented_pairs_from_sparse` (oriented, dual-valid-deduplicated read of an asymmetric product), `pairs_from_groups`, `dedup_pairs` (BFS engine), and the adapter-only `remap_pairs_to_caller`. |
| `_pair_extractor.py` | `MatrixPairExtractor` — exact, path-counting matrix pair extraction returning oriented graph-space blocks for a dependency-closed code set — plus the `relationship_pairs` assembly (selector validation, `dependency_closure`, thread budget, registry-order precedence fold, block construction) and the `check_exclusive` invariant checker (tests, or `PEDIGREE_GRAPH_DEBUG_EXCLUSIVITY=1`). |
