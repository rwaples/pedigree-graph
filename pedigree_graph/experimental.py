...
- :func:`count_pairs_bfs` — BFS / boolean-matmul / numba relationship pair
  counter. Counts-only API. Differs from the matrix engine
  ...
  *paths* (multiplicity). Identical on non-inbred pedigrees.
...
def count_pairs_bfs(
    ...
    *paths* (multiplicity); the four cousin-style codes
    ...
    # Sanity: registry keys should match.
    assert set(named) == set(PAIR_KINSHIP), "BFS produced an unexpected code set"
    return named
