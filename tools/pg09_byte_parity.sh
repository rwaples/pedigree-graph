...
# summary of the same pedigree, written as text. All are rebuilt from scratch
# on the smoke scenario and hashed.
#
...
( cd "$ROOT/fitACE" && pixi run --frozen snakemake --cores 4 -f "$FITACE_TSV" "$FITACE_GRM_BIN" "$FITACE_GRM_ID" )
